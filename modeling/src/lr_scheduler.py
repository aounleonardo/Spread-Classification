...
class LRSchedulerWithRestart(_LRScheduler):
    ...
    def step(self, epoch=None):
        ...
        if (
            ...
            and self.scheduler.last_epoch > 0
            and self._t % self.restart_every == 0
        ):
            self._t = 0
            self.restart_every = int(self.restart_every * self.restart_factor)
            self.scheduler.base_lrs = [
                lr * self.init_lr_factor for lr in self.scheduler.base_lrs
            ]
            if self.verbose:
                print(
                    "LRSchedulerWithRestart: restart lr at epoch %i, next restart at %i"
                    % (
                        self.scheduler.last_epoch,
                        self.scheduler.last_epoch + self.restart_every,
                    )
                )
